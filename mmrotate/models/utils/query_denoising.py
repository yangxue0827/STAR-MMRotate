...
class DnQueryGenerator:
    ...
    def __init__(self,
        ...
        self.angle_noise_scale = noise_scale['angle']
    ...
    def __call__(self,
        ...
        for img_meta, bboxes in zip(img_metas, gt_bboxes):
            ...
            factor = bboxes.new_tensor([img_w, img_h, img_w,
                                        img_h]).unsqueeze(0)
            bboxes_normalized = bboxes[:, :4] / factor
            gt_bboxes_list.append(bboxes_normalized)
            gt_angles_list.append(bboxes[:, 4])
        gt_bboxes = gt_bboxes_list
        gt_angles = gt_angles_list
        ...
        known_angles = angles.repeat(2 * num_groups, 1).view(-1)
        known_labels_expand = known_labels.clone()
        known_bbox_expand = known_bboxs.clone()
        known_angle_expand = known_angles.clone()
        ...
        if self.angle_noise_scale > 0:
            known_angle_ = torch.clone(known_angles)
            rand_sign = torch.randint_like(
                known_angle_, low=0, high=2, dtype=torch.float32)
            rand_sign = rand_sign * 2.0 - 1.0
            rand_part = torch.rand_like(known_angles)
            rand_part[negative_idx] += 1.0
            rand_part *= rand_sign
            known_angle_ += \
        ...
        input_bbox_embed = torch.cat([input_bbox_embed, known_angle_expand.unsqueeze(-1)], dim=-1)
