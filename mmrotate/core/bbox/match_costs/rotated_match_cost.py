...
class AngleL1Cost:
    ...
    def __init__(self, weight=1., angle_version='le90'):
        self.weight = weight
        assert angle_version in ['oc', 'le90', 'le135']
        period = 90 if angle_version == 'oc' else 180
        self.period = period / 180. * math.pi

    def __call__(self, pred, gt):
        pred_up = pred + self.period
        pred_down = pred - self.period
        cost = torch.cdist(pred, gt, p=1)
        cost2 = torch.cdist(pred_up, gt, p=1)
        cost3 = torch.cdist(pred_down, gt, p=1)
        cost = torch.where(cost < cost2, cost, cost2)
        cost = torch.where(cost < cost3, cost, cost3)
