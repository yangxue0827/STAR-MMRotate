...
class ARS_HungarianAssigner(BaseAssigner):
    ...
    def __init__(self,
                 ...
                 angle_cost=dict(type='CrossEntropyLossCost', weight=1.0)):
        self.cls_cost = build_match_cost(cls_cost)
        self.reg_cost = build_match_cost(reg_cost)
        self.iou_cost = build_match_cost(iou_cost)
        self.angle_cost = build_match_cost(angle_cost)
    ...
    def assign(self,
        ...
        angle_cost = self.angle_cost(angle_pred, gt_angles)
        if aspect_ratio is not None:
            aspect_ratio = aspect_ratio.view(1, -1)
            angle_cost *= 2 * aspect_ratio / (1 + aspect_ratio)
        cost = cls_cost + reg_cost + iou_cost + angle_cost
